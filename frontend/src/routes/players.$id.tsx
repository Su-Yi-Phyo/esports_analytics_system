...
function PlayerDetail() {
  ...
  const [timeline, setTimeline] = useState<MongoTimeline[]>([]);
  ...
  useEffect(() => {
    Promise.all([
      ...
      apiGet<MongoTimeline[]>(`/api/mongo/player/${id}/timeline`)
        .catch(() => [])
    ])
      .then(([player, mongoTimeline]) => {
        setP(player);
        setTimeline(mongoTimeline);
      })
      .finally(() => setLoading(false));
  }, [id]);

  const fallbackMatches = p?.recentMatches ?? [];

  const chartSource =
    timeline.length > 0
      ? timeline
      : fallbackMatches.map(m => ({
          match_id: m.matchId ?? m.match_id ?? 0,
  ...
  if (loading) {
    return <div className="p-10 text-muted-foreground">Loading player...</div>;
  }

  if (!p) {
    return <div className="p-10">Player not found</div>;
  }
        ...
        <div className="relative flex flex-col md:flex-row items-start gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2 tracking-widest uppercase">
              <span>{p.team ?? p.team_name ?? "Free Agent"}</span>
              ·
            ...
            <div className="text-muted-foreground">
              {p.realName ?? p.real_name ?? "Unknown Player"} · MongoDB Match Analytics
            </div>
          ...
          <Link
            to="/compare"
            search={{ a: String(p.id) }}
            className="gradient-primary glow-primary px-5 py-3 rounded-lg text-sm font-bold uppercase tracking-widest inline-flex items-center gap-2"
          >
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              MongoDB nested match logs · avg {avg.toFixed(2)}
            </p>
          </div>
          ...
          <LineChart data={data}>
            ...
            <Tooltip
              contentStyle={{
                ...
                border: "1px solid oklch(0.7 0.27 305 / 0.4)",
                borderRadius: 8,
              }}
            />
            ...
            <Line
              ...
              stroke="url(#kdaGr)"
              strokeWidth={2.5}
              dot={{ r: 2, fill: "oklch(0.78 0.18 200)" }}
              activeDot={{ r: 5 }}
            />
          </LineChart>
        </ResponsiveContainer>
  ...
  return (
    <div
      ...
      style={{
        ...
        height: size,
        background:
          "linear-gradient(135deg, oklch(0.7 0.27 305), oklch(0.78 0.18 200))",
        fontSize: size * 0.35,
...
function TrendBadge({
  ...
  return (
    <span
      className={`inline-flex rounded-md border px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider ${cls} ${
